 ...
 * @since  1.6
 */
class PlgSearchContentImages extends JPlugin
{
	/**
	...
	{
		static $areas = array(
			'contentimages' => 'JGLOBAL_ARTICLES'
		);
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null)
	{
		...
		if ($sContent && $limit > 0)
		{
			...
			$query->select('a.title AS title, a.metadesc, a.metakey, a.created AS created, a.language, a.catid. a.images')
		...
		if (count($rows))
		{
			foreach ($rows as $row)
			{
				...
				foreach ($row as $article)
				{
					...
					$article->image_intro         = $images->get('image_intro', null);
					$article->image_intro_alt     = $images->get('image_intro_alt', null);
					$article->image_fulltext      = $images->get('image_fulltext', null);
					$article->image_fulltext_alt  = $images->get('image_fulltext_alt', null);
